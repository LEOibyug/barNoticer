...
- 从屏幕顶部刘海区域展开的待办岛面板。
- 支持窄体和宽体两种岛模式。
- 支持按重要性分组，也支持按自定义分组展示，自定义分组可拖动调整顺序。
- 岛内可快速查看、展开备注、完成事项，并可一键唤出新建事项小面板。
- 新建事项使用独立轻量小面板，支持全局快捷键唤出。
- 事项支持标题、备注、重要性、完成状态、自定义分组、可选截止时间和多时间点安排。
- 支持一次性多时间点事项，也支持每天、每周、每月和每 N 天重复事项。
...
barNoticer 主要由三个界面组成：

- 岛面板：用于快速查看、完成事项、展开备注，并可唤出新建事项小面板。
- 新建事项小面板：用于快速创建事项，支持备注、重要性、分组、DDL、多时间点和重复规则。
...
- 新增事项备注。备注默认不会占用岛面板事项块空间，点击事项块后会展开显示；备注较长时可在展开区域内滚动查看。
- 新建事项流程改为独立小面板。主面板和岛面板都会唤出同一个小面板，不再在岛内直接输入创建。
- 新建事项小面板改为浅色紧凑样式，默认出现在屏幕下半区，并支持 `Esc` 快速关闭。
- 新建事项小面板支持完整时间计划：无时间、单次 DDL、一次性多个时间点、每天/每周/每月/每 N 天重复。
- 新增“新建事项”全局快捷键设置，默认 `⌘⌥N`，可在应用设置中修改。
- 岛面板头部新增 `+` 按钮，用于快速唤出新建事项小面板。
- AI 待办接口同步支持事项备注，AI 可以创建、修改或清空备注，并在上下文中读取备注信息。
- 自定义分组支持持久化排序。主面板左侧分组列表、窄体岛和宽体岛中的分组顺序保持同步。
- 分组排序改为通过拖动 handle 操作，减少误触；宽体岛横向拖动，窄体岛和主面板纵向拖动。
