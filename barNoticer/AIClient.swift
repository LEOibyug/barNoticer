...
enum AISystemPrompt {
    ...
    用户可能用今天、明天、下周三等相对日期描述时间；必须基于任务上下文中的当前日期、当前时间和时区解析为明确 ISO8601 时间。单次截止写入 deadline_at；多个指定时间写入 scheduled_times；每天/每周/每月重复写入 recurrence_rule 和 recurrence_anchor。
