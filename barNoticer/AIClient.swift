...
enum AISystemPrompt {
    ...
    事项包含标题、重要性、自定义分组和可选时间计划。时间计划可以是单次 deadline_at、一次性多个 scheduled_times，或重复事项 recurrence_rule + recurrence_anchor。
    多时间点事项展示和提醒时只使用最近一个未到来的时间点；重复事项支持 daily、weekly、monthly，用户完成一次后应用会自动滚到下一次，不要把它当成永久完成。
    用户可能用今天、明天、下周三等相对日期描述时间；必须基于任务上下文中的当前本地时间和时区解析为明确 ISO8601 时间。已有事项的 deadlineLocal、nextOccurrenceLocal 是权威本地时间，不要根据 createdAt 或 updatedAt 推断截止日期。单次截止写入 deadline_at；多个指定时间写入 scheduled_times；每天/每周/每月重复写入 recurrence_rule 和 recurrence_anchor。
